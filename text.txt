...
void loop() {
  ...
  if (rfid.PICC_IsNewCardPresent() && rfid.PICC_ReadCardSerial()) {
      if (millis() - lastCardReadTime >= cardCooldown) { 
        ...
        if (accessGranted) {
          ...
          if (wrongCardCount >= 5) { isIntrusionActive = true; intrusionTimer = millis(); wrongCardCount = 0; }
        }
      }
      rfid.PICC_HaltA(); 
  }
  ...
  if (millis() - lastFlameCheckTime >= 80) {
    ...
    if (currentFireCondition) { if (!isFireActive) { isFireActive = true; } fireTimer = millis(); }
  }
  ...
  if (millis() - lastSmokeCheckTime >= 500) {
    ...
    if (kitchenSmoke > SMOKE_THRESHOLD || hallwaySmoke > SMOKE_THRESHOLD || livingSmoke > SMOKE_THRESHOLD) { if (!isFireActive) { isFireActive = true; } fireTimer = millis(); }
  }

  if (awayMode) {
    bool intrusionDetected = (hallwayPIR_Triggered || garagePIR_Triggered || reed1 || reed2 || reed3 || vibration);
    if (intrusionDetected && !isIntrusionActive) { isIntrusionActive = true; intrusionTimer = millis(); }
  }

  if (pendingAwayMode && (millis() - awayModeActivationTimer >= 3000)) { pendingAwayMode = false; awayMode = true; playSecurityActivatedSound(); }
  if (isIntrusionActive && (millis() - intrusionTimer >= 10000)) { isIntrusionActive = false; }
  if (isFireActive && (millis() - fireTimer >= 10000)) { isFireActive = false; } 

  if (pump1State && (millis() - pump1Timer >= 10000)) { pump1State = false; }
  if (pump2State && (millis() - pump2Timer >= 10000)) { pump2State = false; }
  if (pump3State && (millis() - pump3Timer >= 10000)) { pump3State = false; }
